...
/**
 * <p>Title: BaseResourceModifyCmdExe </p>
 ...
 */
@Component
public class BaseResourceModifyCmdExe implements CommandCommonExecutorI<Command, Response> {
	...
	public <T extends EntityObject> Response execute(Command cmd, Class<T> clz) {
		...
		Long id = (Long) ServiceContextHolder.get().getContain().get(ServiceContextHolder.get().getClass().getName());
		T entity = this.baseFactory.reBuild(id, clz);
