...
public class CustomizeJarLauncher extends JarLauncher {
	...
	public static AbstractLauncheApplication queryApplication(String code) {
		Optional<AbstractLauncheApplication> optional =  CustomizeJarLauncher.launcheApplicationMap.values().stream().filter((t)->{ return t.getApplicationName().equals(code);}).findFirst();
...
//		System.err.println(" CustomizeJarLauncher class classloader: "+getClass().getClassLoader()+" thread classloader: "+Thread.currentThread().getContextClassLoader());
	...
	public static void closeApplication(Long appId) {
		AbstractLauncheApplication launcheApp = CustomizeJarLauncher.launcheApplicationMap.get(appId);
		 int exitCode =   SpringApplication.exit(launcheApp.getApplicationContext());
         log.info("停止应用  exitCode:"+exitCode);
         
         ((CustomizeLaunchedURLClassLoader)launcheApp.getClassLoader()).clearCache();
        
         CustomizeLaunchedURLClassLoader.closeJarFileArchive((CustomizeLaunchedURLClassLoader)launcheApp.getClassLoader());
         
         launcheApp.destroy();
         CustomizeJarLauncher.launcheApplicationMap.remove(appId);
	}
	...
	public static class LauncheApplication extends AbstractLauncheApplication{
		...
		@Override
		public Object invoke(Class<?> modelClass, String methodName, Map<String, Class<?>> paramsClassMap, Long id,
